...
these numbers exist otherwise nil or null or None or Nothing (depending on the language). In C++ return in such a case
{0, 0}.
...
std::pair <long long, long long> GapInPrimes::gap(int g, long long m, long long n)
{
  ...
  for (auto i = m; i < n; i++)
  {
    ...
    if (couple.size() == 2)
    {
      if (couple.back() - couple.front() == g )
        return std::make_pair(couple.front(),couple.back());
      else
        couple.erase(begin(couple));
    }  
  }
